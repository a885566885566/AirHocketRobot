...
BLOCK* BLOCKBUFFER::getLastBlock()
{
  ...
  {
    tempBlock->nowPoint.x = route.preP.x;
    tempBlock->nowPoint.y = route.preP.y;
    tempBlock->mode=0;
    return tempBlock;
  }
  else
...
void BLOCKBUFFER::refresh()
{
	if(!busy)								//make sure the process won't be interrupt by itself
	{
      ...
      cmd.printInfo();
			if(newCmd.mode == CmdMode_MOVE) //only add new cmd when mode == move
			{
		        ...
		        #ifdef DEBUGGER
		          	Serial.print("BLOCK->new cmd has been loaded suscessfully");
		        #endif
			}
  ...
  int ODx,ODy,OTx,OTy;
			...
			newBlock->mode = 0;				//clear old data
		}
