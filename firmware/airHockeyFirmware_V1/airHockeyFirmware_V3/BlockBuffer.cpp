...
{   
    ...
    blockArray[head].x = (*input).x;
    blockArray[head].y = (*input).y;
...
POINT BLOCKBUFFER::getLastPoint()
{
  POINT lastP;
	if(getCapcity()<=0)                   //no block available
  {
    lastP.x = machinePrePoint.x;
    lastP.y = machinePrePoint.y;
    return lastP;
  }
  else
  {
    lastP.x = blockArray[tail].x;
    lastP.y = blockArray[tail].y;
    #ifdef DEBUGGER
        Serial.print("\nLast block = ");
        //(*tempBlock).printInfo();
    #endif
...
BLOCK* BLOCKBUFFER::pop()
{
  //BLOCK popBlock;
    ...
    /*
     popBlock.mode = blockArray[tail].mode;
    popBlock.dx = blockArray[tail].dx;
    popBlock.dy = blockArray[tail].dy;
    popBlock.tx = blockArray[tail].tx;
    popBlock.ty = blockArray[tail].ty;
    popBlock.x = blockArray[tail].x;
    popBlock.y = blockArray[tail].y;
     */
    
    #ifdef DEBUGGER
      //Serial.print("\npop block = ");
      //printBlockInfo(&popBlock);
    #endif
    //deleteTail();
    //return popBlock;
    return tempBlock;
...
void BLOCKBUFFER::refresh()
{
    ...
    Serial.println("load");
     ...
     //Serial.print("route nowP=");
     //printPointInfo(&route.nowP);
      	//newRoute = true;
     	newBlock.dx = ODx;
     	newBlock.dy = ODy;
     	newBlock.tx = OTx;
    	newBlock.ty = OTy;
      newBlock.x = route.nowP.x;
      newBlock.y = route.nowP.y;
